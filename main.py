...
from dataset import TVSD_Dataset
...
def main(args):
    ...
    layers = [name for name, module in model.named_modules() if len(list(module.children())) == 0 and 'relu' not in name]
    ...
    for layer in hook_layers:
        ...
        layer_dir = f"{args.output_dir}/TVSD/{model_name}/{layer}"
        activations = []
        for chunk_idx in range(max_chunks):
            file_path = f"{layer_dir}/chunk_{chunk_idx}.pt"
            try:
                chunk_activations = torch.load(file_path) # [C, B, ...]
        ...
        layer_score, layer_std = compute_brain_score(X=activations, Y=neural_responses, n_splits=args.n_splits, reducer=args.reducer, correlation_fn=args.correlation_fn)
        layer_scores[layer] = {
            'score': layer_score,
            'std': layer_std
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', type=str, default="/users/jamullik/scratch/TVSD-real/outputs/activations", help='Directory to save activations.')
    ...
    parser.add_argument('--n_splits', type=int, default=10, help='Number of splits for KFold cross-validation.')
