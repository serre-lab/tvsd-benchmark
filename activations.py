...
with h5py.File(train_things_paths_path, "r") as f:
    train_set = f["train_imgs"]
    num_samples = train_set["things_path"].shape[0]
    train_paths = [
        (
            f[train_set["things_path"][i][0]][()]
            .tobytes()
            .decode("utf-16")
            .replace("\\", "/")
        )
...
train_dataset = PathDataset(
    ...
    paths=train_paths,  # Replace with actual paths
...
for layer in layers:
    ...
    save_path = os.path.join(output_dir, f"{layer}_train_activations.pt")
